...
String resource = "";
const int  port = 80;
unsigned long checkConnections = 0;
static int intervalToCheckConnection = 60000; // 60 seconds
unsigned long lastPostCount = 0; //when was the last time sql connection was made
unsigned long postInterval = 10000; //10 seconds closes http.get connection after 10 seconds
...
void loop() {
  ...
  if (digitalRead(triggerPin)) {
    simModuleChecks();
    if ((millis() - previousupdate) > 60000) {
      sendSMSUpdateSQL();
...
void simModuleChecks() {
  ...
  if ((millis() - checkConnections) > intervalToCheckConnection) {
    connectGPRS();
    checkConnections = millis();
  }
...
void takeAction(char whichSensor) {
  if (distance < minDistantce  && distance > 0) {
    sendESP(whichSensor);
